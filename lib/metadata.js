...
function processCsvFiles(bucket, key, sat, transform, cb, currentFileNum=0, lastFileNum=0, arn=null, retries=0) {
...
function update(event, transform, cb) {
  ...
  if (!esClient) {
    es.connect().then((client) => {
      esClient = client;
      es.putMapping(esClient, process.env.ES_INDEX).catch((err) => {})
...
module.exports.update = [update, split]
