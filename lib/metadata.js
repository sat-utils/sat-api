  ...
  return new Promise((resolve, reject) => {
    pump(csvStream, transform, toEs, esStream, function(err) {
          if (err) {
            ...
            console.log(`Finished: ${n_records} records, ${n_transformed} transformed`)
...
function invokeLambda(bucket, key, satellite, currentFileNum, lastFileNum, arn) {
    ...
    if (nextFileNum) {
      ...
      const params = {
        stateMachineArn: arn,
        input: JSON.stringify({ bucket, key, satellite, currentFileNum: nextFileNum, lastFileNum, arn}),
